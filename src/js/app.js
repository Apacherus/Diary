...
~function() {
    ...
    var app = {
        ...
        init: function () {
            ...
            app.EatNote = function (products, title, date) {
                if(typeof products == "undefined") products = [];
                if(!title) title = '';
                var options = {};
                options.type = 'eat';
                options.date = typeof date == "string" ? new Date(date) : ( !date ? new Date() : date );
                options.isGone = false;
                options.notify = true;
                options.created = new Date();
                options.title = title;
                options.products = products;
                return new app.Note(options);
            };

            app.WaterNote = function(volume, date){
                if(!volume) volume = 0;
                var options = {};
                options.date = typeof date == "string" ? new Date(date) : ( !date ? new Date() : date );
                options.type = 'water';
            ...
            app.MeasureNote = function(weight, height, date){
                if(!weight) weight = -1;
                if(!height) height = -1;
                if(!date) date = '';
                var options = {};
